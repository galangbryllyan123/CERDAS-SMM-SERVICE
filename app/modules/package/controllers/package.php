...
class package extends MX_Controller {
	...
	public function index($cate_url_slug = ""){
		...
		if (get_role('admin')) {
			# code...
			$exists_cate = $this->model->get('id, ids, name, url_slug, page_title, meta_keywords, meta_description', $this->tb_categories, ['url_slug' => $cate_url_slug]);
		}else{
			$exists_cate = $this->model->get('id, ids, name, url_slug, page_title, meta_keywords, meta_description', $this->tb_categories, ['url_slug' => $cate_url_slug , 'status' => 1]);
		}
